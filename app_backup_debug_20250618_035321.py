...
class GustBotEnhanced:
    ...
    def setup_misc_routes(self):
        ...
        def get_token_status():
            ...
            token = load_token()
            if token:
                # In real implementation, check token expiry
                return jsonify({
                    'demo_mode': False,
                    'has_token': True,
                    'token_valid': True,
                    'time_left': 3600,  # Placeholder
                    'websockets_available': WEBSOCKETS_AVAILABLE
                })
            else:
                return jsonify({
        ...
        @self.app.route('/api/token/refresh', methods=['POST'])
        def refresh_token():
            """Refresh authentication token"""
            if 'logged_in' not in session:
                return jsonify({'error': 'Authentication required'}), 401
            
            # In real implementation, refresh the G-Portal token
