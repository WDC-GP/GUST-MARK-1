...
class ClansComponent extends BaseComponent {
        ...
        this.container.innerHTML = `
            <div class="clans-management">
                ...
                <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    <!-- Create Clan Section -->
                    ${this.options.enableClanCreation ? this.renderCreateClanSection() : ''}
                ...
                <!-- Clan Statistics -->
                ${this.options.showClanStats ? this.renderStatsSection() : ''}
                
                <!-- Member Management Modal -->
                ${this.options.enableMemberManagement ? this.renderMemberModal() : ''}
            </div>
        `;
    }
    
    renderCreateClanSection() {
        return `
            <div class="bg-gray-800 p-6 rounded-lg">
    ...
    bindEvents() {
        // Create clan events
        if (this.options.enableClanCreation) {
            const createBtn = this.container.querySelector('#createClanBtn');
            const nameInput = this.container.querySelector('#clanName');
            const leaderInput = this.container.querySelector('#clanLeader');
            const serverSelect = this.container.querySelector('#clanServerSelect');
            const descriptionInput = this.container.querySelector('#clanDescription');
            
            this.addEventListener(createBtn, 'click', () => this.createClan());
            this.addEventListener(nameInput, 'input', (e) => {
                this.setState({ newClanName: e.target.value });
                this.updateCharacterCount(e.target, this.options.maxClanNameLength);
            });
            this.addEventListener(leaderInput, 'input', (e) => {
                this.setState({ newClanLeader: e.target.value });
            });
            this.addEventListener(serverSelect, 'change', (e) => {
                this.setState({ newClanServerId: e.target.value });
            });
            this.addEventListener(descriptionInput, 'input', (e) => {
                this.setState({ newClanDescription: e.target.value });
                this.updateCharacterCount(e.target, this.options.maxDescriptionLength);
            });
        }
        
        // Filter and sorting events
        const filterInput = this.container.querySelector('#clanFilter');
        const serverFilter = this.container.querySelector('#serverFilter');
        const refreshBtn = this.container.querySelector('#refreshClansBtn');
        
        this.addEventListener(filterInput, 'input', (e) => {
            this.setState({ filterText: e.target.value });
            this.filterAndSortClans();
        });
        ...
        // Sorting buttons
        const sortButtons = this.container.querySelectorAll('[id^="sortBy"]');
        sortButtons.forEach(button => {
            this.addEventListener(button, 'click', (e) => {
                const sortBy = e.target.id.replace('sortBy', '').toLowerCase();
                this.setSorting(sortBy === 'name' ? 'name' : sortBy === 'members' ? 'memberCount' : 'createdDate');
            });
        });
        
        // Member management modal events
        if (this.options.enableMemberManagement) {
            const closeModalBtn = this.container.querySelector('#closeMemberModal');
            if (closeModalBtn) {
                this.addEventListener(closeModalBtn, 'click', () => this.closeMemberModal());
            }
        }
        
        // Global events
        this.eventBus.on('servers:updated', () => this.updateServerOptions());
        this.eventBus.on('clan:member-updated', () => this.loadData());
    }
    ...
    async loadClans() {
        try {
            const clans = await this.api.clans.list();
            this.setState({ clans });
            this.renderClansList(clans);
        } catch (error) {
            console.error('Failed to load clans:', error);
            this.renderClansList([]);
        }
    }
    
    async createClan() {
        const { newClanName, newClanLeader, newClanServerId, newClanDescription } = this.state;
        
        if (!this.validateClanForm()) {
            return;
        }
        ...
        try {
            ...
            const result = await this.api.clans.create({
                name: newClanName.trim(),
                leader: newClanLeader.trim(),
                serverId: newClanServerId,
                description: newClanDescription.trim()
            });
            
            if (result.success) {
                this.showNotification('Clan created successfully!', 'success');
                this.clearCreateForm();
                this.eventBus.emit('clan:created', { 
                    clanId: result.clanId, 
                    name: newClanName,
                    leader: newClanLeader
                });
                await this.loadData();
            } else {
                this.showNotification(result.error || 'Failed to create clan', 'error');
            }
            ...
            this.handleError(error);
            this.showNotification('Error creating clan', 'error');
        }
    }
    
    validateClanForm() {
        const { newClanName, newClanLeader, newClanServerId } = this.state;
        
        if (!newClanName.trim()) {
            this.showNotification('Please enter a clan name', 'warning');
            return false;
        }
        
        if (newClanName.trim().length < 3) {
            this.showNotification('Clan name must be at least 3 characters', 'warning');
            return false;
        }
        
        if (!newClanLeader.trim()) {
            this.showNotification('Please enter a leader ID', 'warning');
            return false;
        }
        
        if (!newClanServerId) {
            this.showNotification('Please select a server', 'warning');
            return false;
        }
    ...
    renderClansList(clans) {
        const clansList = this.container.querySelector('#clansList');
        
        if (!clansList) return;
        
        const filteredClans = this.getFilteredAndSortedClans(clans || this.state.clans);
            ...
            clansList.innerHTML = `
                <div class="text-gray-400 text-center py-8">
                    ...
                    <div class="text-sm mt-2">
                        ${this.state.filterText || this.state.serverFilter ? 'Try adjusting your filters' : 'Create your first clan above'}
                    </div>
                </div>
            `;
            return;
        ...
        clansList.innerHTML = filteredClans.map(clan => this.renderClanCard(clan)).join('');
        
        // Bind clan card events
        this.bindClanCardEvents();
    }
    
    renderClanCard(clan) {
        const serverName = this.getServerName(clan.serverId);
        const memberCount = clan.memberCount || clan.members?.length || 0;
        const createdDate = new Date(clan.createdDate).toLocaleDateString();
                ...
                <div class="flex items-start justify-between">
                    <div class="flex-1">
                        ...
                        <div class="space-y-1 text-sm text-gray-300">
                            ...
                            ${clan.description ? `<div class="text-xs text-gray-400 mt-2">${clan.description}</div>` : ''}
                        </div>
                    </div>
    ...
    bindClanCardEvents() {
        const actionButtons = this.container.querySelectorAll('.clan-action-btn');
        actionButtons.forEach(button => {
            this.addEventListener(button, 'click', (e) => {
                const action = e.target.dataset.action;
                const clanId = e.target.dataset.clanId;
                this.handleClanAction(action, clanId);
            });
        });
    }
    
    async handleClanAction(action, clanId) {
        const clan = this.state.clans.find(c => c.clanId === clanId);
        if (!clan) return;
        
        switch (action) {
            case 'view':
                this.showClanDetails(clan);
                break;
            case 'manage':
                this.showMemberManagement(clan);
                break;
            case 'delete':
                if (confirm(`Delete clan "${clan.name}"? This action cannot be undone.`)) {
                    await this.deleteClan(clanId);
                }
                break;
        }
    }
    ...
    showClanDetails(clan) {
        this.setState({ selectedClan: clan });
        const detailsContainer = this.container.querySelector('#clanDetails');
        
        const membersList = clan.members ? clan.members.map(member => 
                ...
                <div class="flex items-start justify-between mb-6">
                    <div>
                        ...
                        <div class="text-sm text-gray-400">
                            Led by ${clan.leader} • ${clan.memberCount || 0} members • ${this.getServerName(clan.serverId)}
                        </div>
                    </div>
    ...
    showMemberManagement(clan) {
        if (!this.options.enableMemberManagement) return;
        
        const modal = this.container.querySelector('#memberManagementModal');
        const content = this.container.querySelector('#memberModalContent');
        ...
        const addMemberBtn = content.querySelector('#addMemberBtn');
        const removeButtons = content.querySelectorAll('.remove-member-btn');
        
        this.addEventListener(addMemberBtn, 'click', () => {
            const input = content.querySelector('#addMemberInput');
            this.addClanMember(clan.clanId, input.value.trim());
        });
        
        removeButtons.forEach(button => {
            this.addEventListener(button, 'click', (e) => {
                const member = e.target.dataset.member;
                this.removeClanMember(clan.clanId, member);
            });
        });
    ...
    async addClanMember(clanId, userId) {
        if (!userId) {
        ...
        try {
            const result = await this.api.clans.join(clanId, { userId });
            if (result.success) {
                this.showNotification('Member added successfully', 'success');
                this.closeMemberModal();
                await this.loadData();
            } else {
                this.showNotification(result.error || 'Failed to add member', 'error');
            }
        } catch (error) {
            this.handleError(error);
        }
    ...
    async removeClanMember(clanId, memberId) {
        if (!confirm(`Remove ${memberId} from clan?`)) {
            return;
        }
        
        try {
            const result = await this.api.clans.kick(clanId, { 
                leaderId: this.state.selectedClan?.leader,
                targetId: memberId 
            });
            if (result.success) {
                ...
                this.showNotification(result.error || 'Failed to remove member', 'error');
            }
        } catch (error) {
            this.handleError(error);
        }
    ...
    async deleteClan(clanId) {
        try {
            const result = await this.api.clans.delete(clanId, {
                leaderId: this.state.selectedClan?.leader
            });
            
            if (result.success) {
                this.showNotification('Clan deleted successfully', 'success');
                this.eventBus.emit('clan:deleted', { clanId });
                await this.loadData();
            } else {
                this.showNotification(result.error || 'Failed to delete clan', 'error');
            }
        } catch (error) {
            this.handleError(error);
        }
    ...
    getFilteredAndSortedClans(clans) {
        let filtered = [...clans];
        ...
        // Apply server filter
        if (this.state.serverFilter) {
            filtered = filtered.filter(clan => clan.serverId === this.state.serverFilter);
        }
    ...
    setSorting(sortBy) {
        const newDirection = this.state.sortBy === sortBy && this.state.sortDirection === 'asc' ? 'desc' : 'asc';
        this.setState({ sortBy, sortDirection: newDirection });
        this.filterAndSortClans();
        this.updateSortButtons();
    }
    
    updateSortButtons() {
        const buttons = this.container.querySelectorAll('[id^="sortBy"]');
        buttons.forEach(button => {
            const sortType = button.id.replace('sortBy', '').toLowerCase();
            const isActive = (sortType === 'name' && this.state.sortBy === 'name') ||
                           (sortType === 'members' && this.state.sortBy === 'memberCount') ||
                           (sortType === 'created' && this.state.sortBy === 'createdDate');
            
            button.className = `px-3 py-1 rounded ${isActive ? 'bg-purple-600' : 'bg-gray-600'}`;
        });
    }
    ...
    clearCreateForm() {
        ...
        const form = this.container.querySelector('#createClanSection');
        if (form) {
    ...
    updateCharacterCount(input, maxLength) {
        ...
        if (sibling && sibling.classList.contains('text-xs')) {
            sibling.textContent = `${input.value.length}/${maxLength} characters`;
        }
    }
    
    updateServerOptions() {
        // Refresh server options when servers are updated
        const selects = this.container.querySelectorAll('#clanServerSelect, #serverFilter');
        selects.forEach(select => {
            const currentValue = select.value;
            const isServerFilter = select.id === 'serverFilter';
            
            select.innerHTML = isServerFilter ? 
                '<option value="">All Servers</option>' : 
                '<option value="">Choose a server...</option>';
            
            select.innerHTML += this.renderServerOptions();
    ...
    async loadStats() {
        try {
            const stats = await this.api.clans.getStats();
            this.setState({ stats });
    ...
    renderStats(stats) {
        const statsContainer = this.container.querySelector('#clanStats');
        if (statsContainer) {
    ...
    onDestroy() {
        ...
        this.eventBus.off('clan:member-updated');
    }
}

// Export for module systems
if (typeof module !== 'undefined' && module.exports) {
    module.exports = ClansComponent;
} else {
    window.ClansComponent = ClansComponent;
}