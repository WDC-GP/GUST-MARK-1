 ...
 */
class ServerCard extends BaseComponent {
    constructor(containerId, server, options = {}) {
    ...
    render() {
        ...
        const { compact, showActions, showStatus, showStats, enableSelection } = this.options;
            ...
            <div class="server-card ${compact ? 'server-card--compact' : ''} ${this.state.isSelected ? 'server-card--selected' : ''}"
                ...
                <div class="server-card__header">
                    <div class="server-card__title">
                        <h3 class="server-card__name" title="${server.serverName}">
                            ${this.truncateText(server.serverName, 25)}
                        </h3>
                        ${server.isFavorite ? '<i class="icon-star text-yellow-400" title="Favorite"></i>' : ''}
                    </div>
                    ...
                    <div class="server-info-grid">
                        ...
                        <div class="server-info-item">
                            ...
                            <span class="server-info-value">
                                ${this.getRegionDisplay(server.serverRegion)}
                            </span>
                        </div>
                        ...
                        ${server.description ? `
                            <div class="server-info-item server-info-item--full">
                                ...
                                <span class="server-info-value text-sm text-gray-400">
                                    ${this.truncateText(server.description, 50)}
                                </span>
                            </div>
                        ` : ''}
                        ...
                        ${this.state.lastPing ? `
                            <div class="server-info-item">
                                ...
                                <span class="server-info-value text-xs">
                                    ${this.formatRelativeTime(this.state.lastPing)}
                                </span>
                            </div>
                        ` : ''}
                    </div>
                ...
                <div class="server-card__connection">
                    <div class="connection-indicator ${this.state.isConnected ? 'connection-indicator--active' : ''}">
                        ...
                        <span class="connection-text">
                            ${this.state.isConnected ? 'Live Connected' : 'Not Connected'}
                        </span>
                    </div>
                </div>
                ...
                <div class="server-card__loading ${this.state.actionInProgress ? 'active' : ''}">
                    ...
                    <span class="loading-text">
                        ${this.getActionText(this.state.actionInProgress)}
                    </span>
                </div>
            </div>
        `;
        
        // Initialize status indicator if enabled
        if (showStatus) {
    ...
    bindEvents() {
        ...
        if (this.options.enableSelection) {
            const checkbox = this.container.querySelector('.server-checkbox');
        ...
        this.eventBus.on('server:updated', (data) => {
            if (data.serverId === this.server.serverId) {
                this.updateServer(data.server);
            }
        });
        
        this.eventBus.on('server:status-changed', (data) => {
            if (data.serverId === this.server.serverId) {
                this.updateStatus(data.status);
            }
        });
        
        this.eventBus.on('server:connection-changed', (data) => {
            if (data.serverId === this.server.serverId) {
    ...
    initializeStatusIndicator() {
        const container = this.container.querySelector(`#status-${this.server.serverId}`);
        if (container) {
            this.statusIndicator = new StatusIndicator(container, {
                status: this.server.status || 'unknown',
                showText: true,
                animate: true
            });
        }
    ...
    async pingServer() {
        try {
            const result = await this.api.servers.ping(this.server.serverId);
            
            if (result.success) {
                this.setState({ lastPing: new Date().toISOString() });
                this.updateStatus(result.status);
                this.showNotification('Server pinged successfully', 'success');
                
                // Emit event for other components
                this.eventBus.emit('server:pinged', {
    ...
    openConsole() {
        this.eventBus.emit('navigation:goto', {
            tab: 'console',
            serverId: this.server.serverId
        });
    }
    
    async connectLiveConsole() {
        try {
            const result = await this.api.console.liveConnect({
                serverId: this.server.serverId,
                region: this.server.serverRegion
            });
            
            if (result.success) {
                this.updateConnectionStatus(true);
                this.showNotification('Live console connected', 'success');
                
                this.eventBus.emit('server:live-connected', {
                    serverId: this.server.serverId
                });
            } else {
            ...
            this.showNotification('Failed to connect live console', 'error');
            throw error;
        }
    }
    
    editServer() {
        this.eventBus.emit('server:edit-requested', {
            server: this.server
        });
    }
    ...
    async deleteServer() {
        const confirmed = await this.showConfirmDialog(
            'Delete Server',
            `Are you sure you want to delete "${this.server.serverName}"?\n\nThis action cannot be undone.`
        );
        ...
        try {
            const result = await this.api.servers.delete(this.server.serverId);
            
            if (result.success) {
                this.showNotification('Server deleted successfully', 'success');
                
                this.eventBus.emit('server:deleted', {
                    serverId: this.server.serverId,
                    server: this.server
                });
    ...
    handleSelection(selected) {
        ...
        this.container.classList.toggle('server-card--selected', selected);
        
        this.eventBus.emit('server:selection-changed', {
            serverId: this.server.serverId,
            selected: selected
        });
    }
    ...
    handleCardClick() {
        if (!this.options.enableSelection) {
            // Navigate to server details or console
            this.openConsole();
        }
    }
    ...
    showContextMenu(event) {
        const menuItems = [
            { label: 'Open Console', action: 'console', icon: 'icon-terminal' },
            { label: 'Ping Server', action: 'ping', icon: 'icon-pulse' },
            { label: 'Connect Live', action: 'connect', icon: 'icon-plug' },
            { type: 'separator' },
            { label: 'Edit Server', action: 'edit', icon: 'icon-edit' },
            { label: 'Delete Server', action: 'delete', icon: 'icon-trash', destructive: true }
        ];
    ...
    updateServer(serverData) {
        this.server = { ...this.server, ...serverData };
        this.setState({ server: this.server });
    ...
    updateStatus(status) {
        ...
        // Update last ping time if status changed to online
        if (status === 'online') {
            this.setState({ lastPing: new Date().toISOString() });
        }
    }
    ...
    updateConnectionStatus(connected) {
        ...
        if (indicator) {
            indicator.classList.toggle('connection-indicator--active', connected);
            
            const text = indicator.querySelector('.connection-text');
            if (text) {
                text.textContent = connected ? 'Live Connected' : 'Not Connected';
            }
        }
    ...
    getActionText(action) {
        ...
        return actionTexts[action] || 'Processing...';
    }
    ...
    async showConfirmDialog(title, message) {
        return new Promise((resolve) => {
            this.eventBus.emit('modal:confirm', {
                title,
                message,
    ...
    onDestroy() {
        ...
        this.eventBus.off('server:updated');
        this.eventBus.off('server:status-changed');
        this.eventBus.off('server:connection-changed');
    }
}