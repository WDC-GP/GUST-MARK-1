...
class BaseComponent {
    ...
    constructor(containerId, options = {}) {
        ...
        // Inject dependencies
        this.api = options.api || window.App?.api || this.createMockAPI();
        this.eventBus = options.eventBus || window.App?.eventBus || this.createMockEventBus();
    ...
    async init() {
        ...
        try {
            ...
            // Add component class to container
            this.container.classList.add('component', this.constructor.name.toLowerCase());
    ...
    bindEvents() {
        ...
        this.log('Binding events (base implementation)');
    }
     ...
     */
    async loadData() {
        this.log('Loading data (base implementation)');
        // Base implementation - override in subclasses
    }
     ...
     */
    async onReady() {
        this.container.classList.add('component-ready');
        this.eventBus.emit(`component:ready:${this.constructor.name.toLowerCase()}`, {
            component: this,
    ...
    setState(newState, shouldRender = false) {
        ...
        this.log('State updated:', { oldState, newState: this.state });
     ...
     */
    renderUpdate() {
        this.log('Rendering update (base implementation)');
        // Base implementation - override in subclasses for efficiency
    }
    ...
    showLoader() {
        if (this.options.showLoader) {
            ...
            if (!this.container.querySelector('.component-loader')) {
    ...
    showError(error) {
        ...
        // Show in designated error container if specified
        if (this.options.errorContainer) {
            const errorElement = document.getElementById(this.options.errorContainer);
            if (errorElement) {
        ...
        // Show in component container
        const errorDiv = document.createElement('div');
        errorDiv.className = 'component-error bg-red-800 border border-red-600 text-red-200 p-3 rounded mb-4';
        errorDiv.innerHTML = `
    ...
    hideError() {
        ...
        const errorElements = this.container.querySelectorAll('.component-error');
    ...
    handleError(error, context = 'unknown') {
        ...
        const fullMessage = `Error in ${this.constructor.name} (${context}): ${errorMessage}`;
    ...
    getUserFriendlyError(error) {
        if (error.status) {
            switch (error.status) {
                ...
                case 404: return 'Resource not found. It may have been deleted or moved.';
                case 408: return 'Request timeout. Please check your connection and try again.';
    ...
    showNotification(message, type = 'info') {
        this.eventBus.emit('notification:show', { message, type, source: this.constructor.name });
    }
     ...
     */
    log(...args) {
        if (this.options.debugMode || (window.App && window.App.debugMode)) {
    ...
    destroy() {
        ...
        this.eventBus.emit(`component:destroyed:${this.constructor.name.toLowerCase()}`, {
            component: this,
            containerId: this.containerId
        });
    ...
    async refresh() {
        ...
        try {
            ...
            this.log('Component refreshed successfully');
        } catch (error) {
     ...
     */
    isInState(stateName) {
        return Boolean(this.state[stateName]);
    }
