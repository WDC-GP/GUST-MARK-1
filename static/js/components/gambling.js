...
class GamblingComponent extends BaseComponent {
        ...
        this.container.innerHTML = `
            <div class="gambling-games">
                ...
                <div class="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-6">
                    ${this.options.enableSlots ? this.renderSlotsGame() : ''}
                    ${this.options.enableCoinflip ? this.renderCoinflipGame() : ''}
                    ${this.options.enableDice ? this.renderDiceGame() : ''}
                </div>
    ...
    bindEvents() {
        ...
        if (this.options.enableSlots) {
            const spinBtn = this.container.querySelector('#spinSlotsBtn');
            const slotsUserInput = this.container.querySelector('#slotsUserId');
            const slotsBetInput = this.container.querySelector('#slotsBet');
            
            this.addEventListener(spinBtn, 'click', () => this.playSlots());
            this.addEventListener(slotsUserInput, 'input', (e) => {
                this.setState({ slotsUserId: e.target.value });
            });
            this.addEventListener(slotsBetInput, 'input', (e) => {
                this.setState({ slotsBet: parseInt(e.target.value) || 0 });
            });
        }
        
        // Coinflip events
        if (this.options.enableCoinflip) {
            const flipBtn = this.container.querySelector('#flipCoinBtn');
            const coinflipUserInput = this.container.querySelector('#coinflipUserId');
            const coinflipBetInput = this.container.querySelector('#coinflipBet');
            const coinflipChoiceSelect = this.container.querySelector('#coinflipChoice');
            
            this.addEventListener(flipBtn, 'click', () => this.playCoinflip());
            this.addEventListener(coinflipUserInput, 'input', (e) => {
                this.setState({ coinflipUserId: e.target.value });
            });
            this.addEventListener(coinflipBetInput, 'input', (e) => {
                this.setState({ coinflipBet: parseInt(e.target.value) || 0 });
            });
            this.addEventListener(coinflipChoiceSelect, 'change', (e) => {
                this.setState({ coinflipChoice: e.target.value });
            });
        }
        
        // Dice events
        if (this.options.enableDice) {
            const rollBtn = this.container.querySelector('#rollDiceBtn');
            const diceUserInput = this.container.querySelector('#diceUserId');
            const diceBetInput = this.container.querySelector('#diceBet');
            const dicePredictionSelect = this.container.querySelector('#dicePrediction');
            
            this.addEventListener(rollBtn, 'click', () => this.playDice());
            this.addEventListener(diceUserInput, 'input', (e) => {
                this.setState({ diceUserId: e.target.value });
            });
            this.addEventListener(diceBetInput, 'input', (e) => {
                this.setState({ diceBet: parseInt(e.target.value) || 0 });
            });
            this.addEventListener(dicePredictionSelect, 'change', (e) => {
                this.setState({ dicePrediction: parseInt(e.target.value) });
            });
        }
    ...
    async playSlots() {
        const { slotsUserId, slotsBet } = this.state;
        
        if (!this.validateGameInput(slotsUserId, slotsBet, 'slots')) {
            return;
        }
        
        if (this.state.slotsSpinning) {
            return; // Prevent multiple spins
        }
        
        try {
            this.setState({ slotsSpinning: true });
            this.animateSlots();
            
            const result = await this.api.gambling.playSlots({
                userId: slotsUserId,
                bet: slotsBet
            });
            ...
            if (result.success) {
                ...
                setTimeout(() => {
                    this.setState({ slotsResult: result, slotsSpinning: false });
                    this.displaySlotsResult(result);
                    this.eventBus.emit('gambling:game-played', { 
                        game: 'slots', 
                        userId: slotsUserId, 
                        result 
                    });
                ...
                this.setState({ slotsSpinning: false });
                this.showNotification(result.error || 'Slots game failed', 'error');
            }
    ...
    async playCoinflip() {
        const { coinflipUserId, coinflipBet, coinflipChoice } = this.state;
        
        if (!this.validateGameInput(coinflipUserId, coinflipBet, 'coinflip')) {
            return;
        }
        ...
        try {
            this.setState({ coinflipFlipping: true });
            this.animateCoinflip();
            
            const result = await this.api.gambling.playCoinflip({
                userId: coinflipUserId,
                amount: coinflipBet,
            ...
            if (result.success) {
                // Wait for animation to complete
                setTimeout(() => {
                    this.setState({ coinflipResult: result, coinflipFlipping: false });
                    this.displayCoinflipResult(result);
                    this.eventBus.emit('gambling:game-played', { 
                        game: 'coinflip', 
                        userId: coinflipUserId, 
                        result 
                ...
                this.setState({ coinflipFlipping: false });
                this.showNotification(result.error || 'Coinflip game failed', 'error');
            }
    ...
    async playDice() {
        const { diceUserId, diceBet, dicePrediction } = this.state;
        
        if (!this.validateGameInput(diceUserId, diceBet, 'dice')) {
            return;
        }
        ...
        try {
            ...
            const result = await this.api.gambling.playDice({
                userId: diceUserId,
                amount: diceBet,
            ...
            if (result.success) {
                // Wait for animation to complete
                setTimeout(() => {
                    this.setState({ diceResult: result, diceRolling: false });
                ...
                this.showNotification(result.error || 'Dice game failed', 'error');
            }
    ...
    validateGameInput(userId, betAmount, gameType) {
        if (!userId.trim()) {
            this.showNotification('Please enter a user ID', 'warning');
            return false;
        }
        
        if (!betAmount || betAmount < this.options.minBetAmount) {
            this.showNotification(`Minimum bet is ${this.options.minBetAmount}`, 'warning');
            return false;
        }
        
        if (betAmount > this.options.maxBetAmount) {
            this.showNotification(`Maximum bet is ${this.options.maxBetAmount}`, 'warning');
            return false;
        }
    ...
    animateCoinflip() {
        const coin = this.container.querySelector('#coinAnimation');
        const symbols = ['🟡', '⚪'];
        
        let count = 0;
        const interval = setInterval(() => {
            coin.textContent = symbols[count % 2];
            count++;
            if (count >= 20) {
                clearInterval(interval);
            }
    ...
    animateDice() {
        const dice = this.container.querySelector('#diceAnimation');
        const symbols = ['⚀', '⚁', '⚂', '⚃', '⚄', '⚅'];
        
        let count = 0;
        const interval = setInterval(() => {
            dice.textContent = symbols[Math.floor(Math.random() * symbols.length)];
            count++;
            if (count >= 20) {
                clearInterval(interval);
            }
    ...
    displaySlotsResult(result) {
        const resultDiv = this.container.querySelector('#slotsResult');
        const reels = this.container.querySelectorAll('.reel');
        
        // Update reels with actual result
        ...
        const isWin = result.net_change > 0;
        resultDiv.innerHTML = `
    ...
    displayCoinflipResult(result) {
        const resultDiv = this.container.querySelector('#coinflipResult');
        const coin = this.container.querySelector('#coinAnimation');
        
        // Update coin with result
        coin.textContent = result.result === 'heads' ? '🟡' : '⚪';
    ...
    displayDiceResult(result) {
        const resultDiv = this.container.querySelector('#diceResult');
        const dice = this.container.querySelector('#diceAnimation');
        
        // Update dice with result
        const diceSymbols = ['⚀', '⚁', '⚂', '⚃', '⚄', '⚅'];
        dice.textContent = diceSymbols[result.result - 1];
    ...
    async loadStats() {
        try {
            // Implementation would fetch gambling statistics
            const stats = {
                totalGames: 0,
                totalWagered: 0,
                totalWon: 0,
            ...
            console.error('Failed to load gambling stats:', error);
        }
    }
    
    async loadLeaderboard() {
        try {
            const leaderboard = await this.api.gambling.getLeaderboard();
            this.setState({ leaderboard });
            this.renderLeaderboard(leaderboard);
        } catch (error) {
            console.error('Failed to load gambling leaderboard:', error);
        }
    }
    
    async loadHistory() {
        try {
            // Implementation would fetch recent gambling history
            const history = [];
            this.setState({ history });
            this.renderHistory(history);
        } catch (error) {
            console.error('Failed to load gambling history:', error);
        }
    }
    
    renderStats(stats) {
        const statsContainer = this.container.querySelector('#gamblingStats');
        if (statsContainer) {
    ...
    renderLeaderboard(leaderboard) {
        const leaderboardContainer = this.container.querySelector('#gamblingLeaderboard');
        if (leaderboardContainer) {
            if (leaderboard.length === 0) {
                leaderboardContainer.innerHTML = '<div class="text-gray-400 text-center py-4">No games played yet</div>';
                return;
            }
            
            leaderboardContainer.innerHTML = leaderboard.slice(0, 5).map((user, index) => `
    ...
    renderHistory(history) {
        const historyContainer = this.container.querySelector('#gamblingHistory');
        if (historyContainer) {
            if (history.length === 0) {
                historyContainer.innerHTML = '<div class="text-gray-400 text-center py-4">No recent games</div>';
                return;
            }
            
            // Implementation would render history table
            historyContainer.innerHTML = '<div class="text-gray-400 text-center py-4">History feature coming soon</div>';
        }
    }
    
    onBalanceUpdated(data) {
        // Handle balance updates to refresh any displays if needed
    }
...
if (typeof module !== 'undefined' && module.exports) {
    module.exports = GamblingComponent;
} else {
    window.GamblingComponent = GamblingComponent;
}