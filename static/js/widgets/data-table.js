...
class DataTable extends BaseComponent {
    ...
    render() {
        const { 
            responsive, striped, hover, bordered, compact, stickyHeader,
            showHeader, showFooter, pagination, selectable, searchable
        } = this.options;
    ...
    bindEvents() {
        ...
        rows.forEach(row => {
            this.addEventListener(row, 'click', (e) => {
                if (!e.target.closest('.row-actions') && !e.target.closest('.row-checkbox')) {
                    this.handleRowClick(row.dataset.rowId, e);
                }
            });
        });
        
        // Row actions
        const actionButtons = this.container.querySelectorAll('.row-action-btn');
        actionButtons.forEach(button => {
            this.addEventListener(button, 'click', (e) => {
                e.stopPropagation();
                this.handleRowAction(button.dataset.action, button.dataset.rowId);
            });
        });
        
        // Bulk actions
        const bulkActionButtons = this.container.querySelectorAll('.bulk-action-btn');
        bulkActionButtons.forEach(button => {
            this.addEventListener(button, 'click', () => {
                this.handleBulkAction(button.dataset.action);
            });
        });
        ...
        if (exportBtn) {
            this.addEventListener(exportBtn, 'click', () => this.exportData());
        }
    ...
    renderHeader() {
        const { columns, selectable, multiSelect } = this.options;
    ...
    renderBody() {
        ...
        return this.paginatedData.map((row, index) => this.renderRow(row, index)).join('');
    }
                ...
                <div class="data-table-info">
                    <span class="data-count">
                        ...
                        ${this.filteredData.length !== this.state.data.length ? 
                            `(filtered from ${this.state.data.length} total)` : ''}
                    </span>
    ...
    renderBulkActions() {
        const { bulkActions, selectable } = this.options;
    ...
    formatCellValue(row, column) {
        ...
        if (typeof value === 'boolean') {
            return value ? '<i class="icon-check text-success"></i>' : '<i class="icon-x text-error"></i>';
        }
    ...
    processData() {
        ...
        if (this.state.searchText) {
            data = this.filterDataBySearch(data, this.state.searchText);
        }
        
        // Apply column filters
        data = this.filterDataByColumns(data, this.state.filters);
        ...
        // Apply pagination
        this.paginatedData = this.paginateData(data);
        
        // Update display
        this.updateDisplay();
    }
    
    filterDataBySearch(data, searchText) {
    ...
    filterDataByColumns(data, filters) {
        return data.filter(row => {
    ...
    calculatePagination() {
        ...
        if (this.state.currentPage > totalPages) {
            this.setState({ currentPage: Math.max(1, totalPages) });
        }
    }
    ...
    updateDisplay() {
        ...
        if (this.options.showFooter) {
            ...
            if (footer) {
                footer.innerHTML = this.renderFooter().replace(/<\/?div[^>]*>/g, '');
            }
        }
    ...
    updateBulkActions() {
        ...
        if (bulkActions) {
            bulkActions.classList.toggle('bulk-actions--visible', this.selectedRows.size > 0);
            
            const label = bulkActions.querySelector('.bulk-actions-label');
            if (label) {
                label.textContent = `${this.selectedRows.size} selected`;
            }
        }
    }
    
    updateEmptyState() {
        const emptyState = this.container.querySelector('#table-empty');
        if (emptyState) {
            const showEmpty = this.paginatedData.length === 0 && !this.state.loading;
            emptyState.classList.toggle('active', showEmpty);
        }
    }
    ...
    handleSearch(searchText) {
        ...
        if (this.options.onFilter) {
            this.options.onFilter({ search: searchText });
        }
    }
    ...
    handleRowClick(rowId, event) {
        if (this.options.onRowClick) {
            ...
            this.options.onRowClick(row, rowId, event);
        }
    }
    
    handleRowSelection(rowId, selected) {
        if (selected) {
        ...
        if (this.options.onRowSelect) {
            const selectedData = Array.from(this.selectedRows).map(id => this.findRowById(id));
    ...
    handleRowAction(action, rowId) {
        ...
        if (actionConfig && actionConfig.handler) {
            actionConfig.handler(row, rowId);
        }
    }
    
    handleBulkAction(action) {
        const actionConfig = this.options.bulkActions.find(a => a.key === action);
        const selectedData = Array.from(this.selectedRows).map(id => this.findRowById(id));
        
        if (actionConfig && actionConfig.handler) {
            actionConfig.handler(selectedData, this.selectedRows);
        }
    }
    ...
    removeRow(rowId) {
        const newData = this.state.data.filter((row, index) => {
            return this.getRowId(row, index) !== rowId;
        });
        this.updateData(newData);
    }
    
    updateRow(rowId, newData) {
        const data = this.state.data.map((row, index) => {
            return this.getRowId(row, index) === rowId ? { ...row, ...newData } : row;
        });
    ...
    setLoading(loading) {
        ...
        const loadingOverlay = this.container.querySelector('#table-loading');
        if (loadingOverlay) {
    ...
    getSortIndicatorClass(column) {
        if (this.state.sortColumn === column) {
            return `sort-indicator--${this.state.sortDirection}`;
        }
        return '';
    }
    ...
    getDisplayRange() {
        ...
        const end = Math.min(currentPage * pageSize, this.filteredData.length);
