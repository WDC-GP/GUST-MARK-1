...
class Modal extends BaseComponent {
    ...
    render() {
        const { title, type, size, closable, showHeader, showFooter, position, animation } = this.options;
        
        const sizeClass = `modal--${size}`;
        const typeClass = `modal--${type}`;
        const positionClass = `modal--${position}`;
        const animationClass = `modal--${animation}`;
    ...
    renderHeader() {
        const { title, closable, type } = this.options;
        const typeIcon = this.getTypeIcon(type);
        
        return `
            <div class="modal-header">
                <div class="modal-header__content">
                    ${typeIcon ? `<div class="modal-header__icon">${typeIcon}</div>` : ''}
    ...
    renderFormContent() {
        ...
        return `
            <form class="modal-form" id="modal-form">
                ${formData.fields.map(field => this.renderFormField(field)).join('')}
            </form>
        `;
    }
    ...
    renderFormField(field) {
        ...
        const errorMessage = this.state.validationErrors[name];
        ...
        switch (type) {
                ...
                return `
                    <div class="form-field ${hasError ? 'form-field--error' : ''}">
                        ...
                        <input type="${type}" 
                               ...
                               value="${value || ''}"
                               ${required ? 'required' : ''}>
                        ${hasError ? `<div class="form-error">${errorMessage}</div>` : ''}
                    </div>
                `;
            ...
            default:
                return `<div class="form-field">Unknown field type: ${type}</div>`;
        }
    }
    
    renderFooter() {
        const { type, actions } = this.options;
        ...
        switch (type) {
            ...
            case 'form':
                defaultActions = [
                    { label: 'Cancel', action: 'cancel', variant: 'secondary' },
                    { label: 'Submit', action: 'submit', variant: 'primary', type: 'submit' }
                ];
                break;
            default:
                defaultActions = [
                ...
                <div class="modal-actions">
                    ${finalActions.map(action => `
                        <button class="btn btn--${action.variant || 'secondary'} ${action.loading ? 'btn--loading' : ''}"
                                ...
                                ${action.type ? `type="${action.type}"` : 'type="button"'}
    ...
    bindEvents() {
        ...
        actionButtons.forEach(button => {
            this.addEventListener(button, 'click', (e) => {
                this.handleAction(button.dataset.action, e);
            });
        });
        ...
        if (form) {
            ...
            inputs.forEach(input => {
                this.addEventListener(input, 'blur', () => this.validateField(input.name));
                this.addEventListener(input, 'input', () => this.clearFieldError(input.name));
            });
        }
    }
    ...
    async close() {
        if (!this.state.isOpen || this.options.persistent) return;
        
        this.setState({ isAnimating: true });
        
        // Call onClose callback
        if (this.options.onClose) {
            const shouldClose = await this.options.onClose(this);
            ...
            switch (action) {
                ...
                default:
                    ...
                    if (this.options.onAction) {
                        await this.options.onAction(action, this);
                    }
            }
        } catch (error) {
    ...
    validateForm(data) {
        ...
        if (!formData || !formData.fields) return errors;
    ...
    manageFocus() {
        const modal = this.container.querySelector('#modal-dialog');
        if (!modal) return;
    ...
    restoreFocus() {
        if (this.focusTrap && this.focusTrap.previouslyFocused) {
    ...
    getTypeIcon(type) {
        const icons = {
            confirm: '<i class="icon-alert-triangle"></i>',
            alert: '<i class="icon-info"></i>',
            form: '<i class="icon-edit"></i>',
    ...
    static form(title, formConfig, options = {}) {
        return new Promise((resolve, reject) => {
            const modal = new Modal('modal-container', {
                ...
                onCancel: () => reject(new Error('Form cancelled')),
                onClose: () => reject(new Error('Form cancelled')),
                ...options
            });
    ...
    static custom(title, content, options = {}) {
        const modal = new Modal('modal-container', {
            title,
            content,
            type: 'custom',
